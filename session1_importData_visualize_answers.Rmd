---
title: "Session 1: Introduction and Visualisation (Answers)"
subtitle: "David Munoz Tord & Denis Mongin"
...
barplot <- ggplot(data = il18, aes(dis.act)) +  #here we only need on variable so ggplot will automatically assign it to
# to the x-axis
 geom_bar()
...
barplot + geom_bar(aes(y = ..prop.., group = 1)) + 
...
ggplot(data = il18, aes(dis.act, fill = levelFreeIL18)) +
  geom_bar()
```
...
hist <- ggplot(data = il18, aes(freeIL18)) + 
  geom_histogram()
...
boxplot <- ggplot(il18, aes(x = dis.act, y = freeIL18)) + #here the is no more fuss
  geom_boxplot() + 
  xlab("Disease activity")
...
#hint: ggplot(data, aes(...)) + geom_violin()
#hint2: assign your plot to an object, e.g. violin <- ggplot(data, aes(...)) + geom_violin()
...
scatter <- ggplot(data = il18, aes(x = freeIL18, y = TotalIL18)) +
  geom_point()
...
ggplot(data = il18, aes(x = freeIL18, y = TotalIL18, color = dis.act)) +
  geom_point()
