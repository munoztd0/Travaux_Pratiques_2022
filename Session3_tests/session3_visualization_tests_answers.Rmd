...
Ok I will try to summarise the data handling knowledge needed here at the begining:
...
We can put this variables in a vector

```{r}
x <- c(3,2,4,5)
x
```
x is a vector of 4 numbers. I can select these elements with squared brackets. here I select the first element:
...
x[c(1,3)]
```
If we put several vectors of same length together, we have a table, called data frame in R:
...
I can laso select lines using a boolean vector, i.e. a vector of true and false. Her I select the first line:

```{r}
df_bisphenol[c(TRUE,FALSE,FALSE),]
```
Why ? Because you can then use boolean operation to select lines. Here the boolean: group is equal to "Control":
...
```
This operation reads: the vector df_bisphenol$group has value within c("Control","High exposition"). The result says "yes, no, yes". You can use that to select lines:

```{r}
df_bisphenol[df_bisphenol$group %in% c("Control","High exposition"),]
```
an other example where it makes more sense (with df_bisphenol you could just use the line number): for the table Bisphenol_long, which contains a result with one line per patient (so 90 lines), we want to keep only the group "Control" and "High_expo"
